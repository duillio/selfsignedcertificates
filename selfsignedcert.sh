...
openssl req -new -x509 -days 365 -subj "/CN=Local CA" -key "${certdir}/ca-key.pem" -sha256 -out "${certdir}/ca.pem"
...
openssl req -subj "/CN=${host}" -new -key "${certdir}/key.pem" -out "${certdir}/${host}.csr"
